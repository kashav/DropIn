...
curl 'https://raw.githubusercontent.com/cobalt-uoft/datasets/master/buildings.json' | jq --slurp 'map(select(.campus == "UTSG")) | map({id, code, lat, lng}) | .[] | {(.code): .}' | jq --slurp add -c > ../app/data/buildings.json
